...
class DVModelerFrameHelper {
    ...
    private PropertyChangeMediator propertyChangeMediator = new PropertyChangeMediator();
    ...
    private void exitAction(WindowEvent e) {
        prefs.putInt("y", dvModelerFrame.getY());
        prefs.putInt("x", dvModelerFrame.getX());
        prefs.putInt("width", dvModelerFrame.getWidth());
        prefs.putInt("height", dvModelerFrame.getHeight());

        if (cayenneProject != null) {

            prefs.put("cayenneProject", cayenneProject.getProjectFile()
                    .toString());

            int returnVal = JOptionPane.showConfirmDialog(dvModelerFrame,
                    "Save current project before exit?", "Exit Confirmation",
                    JOptionPane.YES_NO_CANCEL_OPTION,
                    JOptionPane.QUESTION_MESSAGE);

            if (returnVal == JOptionPane.YES_OPTION) {
                int selectedValue = saveCayenneProjectFileAction(null,
                        SaveErrorsDialog.EXIT_DIALOG);
                if ((selectedValue == -1)
                        || (selectedValue == SaveErrorsDialog.EXIT_WITHOUT_SAVING)) {
                    System.exit(0);
                }
    ...
    private void closeCayenneProject() {
        ...
        cayenneProject = null;
        DataMapTreeModel dataMapTreeModel = (DataMapTreeModel) dataMapTree
                .getModel();
        dataMapTreeModel.setDataMaps(null);

        DataViewTreeModel dataViewTreeModel = (DataViewTreeModel) dataViewTree
                .getModel();
    ...
    private void enableControls() {
        ...
        Object node = (dataViewTabSelected ? dataViewTree
                .getLastSelectedPathComponent() : dataMapTree
                .getLastSelectedPathComponent());
        addNewDataViewAction.setEnabled(openProject && dataViewTabSelected);
        addNewObjEntityViewAction.setEnabled(node instanceof DataView
                || node instanceof DVObjEntity);
        addNewFieldAction.setEnabled(node instanceof ObjEntityView);
        deleteAction.setEnabled(node instanceof DataView
    ...
    private class OpenAction extends AbstractAction {
        private OpenAction() {
            super("Open Cayenne Project...", new ImageIcon(SaveAction.class
    ...
    private void openCayenneProjectFileAction(ActionEvent event) {
        ...
        int returnVal = openProjectFileChooser.showDialog(dvModelerFrame,
                "Open Project");

        if (returnVal == JFileChooser.APPROVE_OPTION) {
            if (cayenneProject != null) {
                int result = JOptionPane.showConfirmDialog(dvModelerFrame,
                ...
                if (result == JOptionPane.CANCEL_OPTION)
                    ...
                    result = saveCayenneProjectFileAction(null,
                            SaveErrorsDialog.CLOSE_DIALOG);
                if ((result == -1)
                        || (result == SaveErrorsDialog.CLOSE_WITHOUT_SAVING)) {
                    cayenneProject = null;
                    dvModelerFrame.getMenuItemLoadErrors().setEnabled(false);
                    dvModelerFrame.getMenuItemSaveErrors().setEnabled(false);
                } else
            ...
            try {
                ...
                if (loadErrorsList.size() != 0) {
                    loadErrorsDialog = new ErrorsDialog(dvModelerFrame,
                            loadErrorsList, "Load Errors");
                ...
                FieldsTableModel tableModel = (FieldsTableModel) fieldsTable
                        .getModel();
                dvModelerFrame.getTopCardPanel().setModels(dataMapTreeModel,
                        dataViewTreeModel, tableModel);
                for (Iterator i = cayenneProject.getDataViews().iterator(); i
                        .hasNext();) {
                    DataView dataView = (DataView) i.next();
                    dataView.addPropertyChangeListener(propertyChangeMediator);
                    for (Iterator j = dataView.getObjEntityViews().iterator(); j
                            .hasNext();) {
                        ObjEntityView view = (ObjEntityView) j.next();
                        view.addPropertyChangeListener(propertyChangeMediator);
                        for (Iterator k = view.getObjEntityViewFields()
                                .iterator(); k.hasNext();) {
                            ObjEntityViewField field = (ObjEntityViewField) k
                                    .next();
                            field
                                    .addPropertyChangeListener(propertyChangeMediator);
                        }
                    }
                }
            } catch (DVModelerException ex) {
                showExceptionDialog("Could not open selected Cayenne project",
                        ex);
            } finally {
    ...
    private class AddNewDataViewAction extends AbstractAction {
        private AddNewDataViewAction() {
            super("Add Data View", new ImageIcon(DVModelerFrame.class
                    .getResource("images/add-dataview.gif")));
            setEnabled(false);
    ...
    private class AddNewObjEntityViewAction extends AbstractAction {
        private AddNewObjEntityViewAction() {
            super("Add ObjEntityView", new ImageIcon(
                    AddNewObjEntityViewAction.class
                            .getResource("images/add-objentityview.gif")));
            setEnabled(false);
    ...
    private void addNewObjEntityViewAction(ActionEvent event) {
        ...
        if (tabIndex == 0) {
            ...
            DVObjEntity selectedObjEntity = (DVObjEntity) JOptionPane
                    .showInputDialog(dvModelerFrame, "Select ObjEntity",
                            "New Entity View", JOptionPane.PLAIN_MESSAGE, null,
                            cayenneProject.getObjEntities(), cayenneProject
                                    .getObjEntities()[0]);
            if (selectedObjEntity != null) {
                view = new ObjEntityView(dataView);
                view.setName(selectedObjEntity.getName() + "View");
                view.setObjEntity(selectedObjEntity);

                for (int i = 0; i < selectedObjEntity.getObjAttributes().size(); i++) {
                    DVObjAttribute objAttribute = (DVObjAttribute) selectedObjEntity
                            .getObjAttributes().get(i);
                    ObjEntityViewField oevf = new ObjEntityViewField(view);
                    oevf.setName(objAttribute.getName());
                    oevf.setPrefIndex(i);
                    oevf.setObjAttribute(objAttribute);
                }
            ...
            if (dataViews.length == 0) {
                JOptionPane.showMessageDialog(dvModelerFrame,
                        "First, create a data view (Tab \"Views\")", "Warning",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }
            DataView selectedDataView = (DataView) JOptionPane.showInputDialog(
                    dvModelerFrame, "Select Data View", "New Entity View",
                    JOptionPane.PLAIN_MESSAGE, null, dataViews, dataViews[0]);
            if (selectedDataView == null)
                return;
            java.util.List allObjEntityViews = new ArrayList();

            for (Iterator j = cayenneProject.getDataViews().iterator(); j
                    .hasNext();) {
    ...
    private class AddNewFieldAction extends AbstractAction {
        private AddNewFieldAction() {
            super("Add Field", new ImageIcon(AddNewFieldAction.class
                    .getResource("images/add-objentityviewfield.gif")));
            setEnabled(false);
            putValue(Action.ACTION_COMMAND_KEY, "create-new-field");
            putValue(Action.ACCELERATOR_KEY, KeyStroke
                    .getKeyStroke("control F"));
            putValue(Action.SHORT_DESCRIPTION,
                    "Create New Field in ObjEntityView");
        }
    ...
    private void addNewFieldAction(ActionEvent event) {
        ...
        ObjEntityView objEntityView = (ObjEntityView) selectionPath
    ...
    private class SaveAction extends AbstractAction {
        private SaveAction() {
            super("Save Cayenne Project", new ImageIcon(SaveAction.class
                    .getResource("images/save_edit.gif")));
            setEnabled(true);
     ...
     * It return -1 if
     */
    private int saveCayenneProjectFileAction(ActionEvent event,
            String dialogType) {
        if (cayenneProject == null)
            return -1;
        try {
            ...
            if (saveErrorsList.size() != 0) {
                saveErrorsDialog = new ErrorsDialog(dvModelerFrame,
                        saveErrorsList, "Save Errors");
                saveErrorsDialog.setVisible(false);
                dvModelerFrame.getMenuItemSaveErrors().setEnabled(true);

                int selectedValue = SaveErrorsDialog.showSaveErrorsDialog(
                        dvModelerFrame, saveErrorsList, dialogType);
            ...
            showExceptionDialog("Could not save Cayenne project", ex);
        }
        return -1;
    }
    ...
    private class AboutAction extends AbstractAction {
        private AboutAction() {
            super("About DataView Modeler");
            setEnabled(true);
            putValue(Action.ACTION_COMMAND_KEY, "about");
            putValue(Action.SHORT_DESCRIPTION, "About DataView Modeler");
    ...
    private class DeleteAction extends AbstractAction {
        private DeleteAction() {
            super("Delete", new ImageIcon(AddNewObjEntityViewAction.class
    ...
    private void deleteAction(ActionEvent e) {
        ...
        if (node instanceof DataView) {
            ...
            if (dataView.getObjEntityViewCount() != 0) {
                int option = JOptionPane.showConfirmDialog(dvModelerFrame,
                        "Do you really whant to delete the selected DataView?",
                        "Deleting DataView", JOptionPane.OK_CANCEL_OPTION);
                ...
                if (option == JOptionPane.OK_OPTION) {
                    ...
                    if (cayenneProject.getDataViews().size() == 0) {
                        ...
                        if (index == 0) {
                            selectDataView((DataView) cayenneProject
                                    .getDataViews().get(0));
                        } else {
                            selectDataView((DataView) cayenneProject
                                    .getDataViews().get(index - 1));
                        }
                    }
                }
            } else {
                ...
                if (cayenneProject.getDataViews().size() == 0) {
                    ...
                    if (index == 0) {
                        selectDataView((DataView) cayenneProject.getDataViews()
                                .get(0));
                    } else {
                        selectDataView((DataView) cayenneProject.getDataViews()
                                .get(index - 1));
            ...
            if (entity != null) {
                ...
                DataMapTreeModel dataMapTreeModel = (DataMapTreeModel) dataMapTree
                        .getModel();
                dataMapTreeModel.objEntityViewRemoved(entity, view,
                        indexInDataMap);
            ...
            if (dataViewTabSelected) {
                if (dataView.getObjEntityViewCount() == 0) {
                    ...
                    if (indexInDataView == 0) {
                        selectObjEntityView(dataViewTabSelected, dataView
                                .getObjEntityView(0));
                    } else {
                        selectObjEntityView(dataViewTabSelected, dataView
                                .getObjEntityView(indexInDataView - 1));
                    }
                }
            } else {
                if (entity.getObjEntityViewCount() == 0) {
                    ...
                    if (indexInDataMap == 0) {
                        selectObjEntityView(dataViewTabSelected, entity
                                .getObjEntityView(0));
                    } else {
                        selectObjEntityView(dataViewTabSelected, entity
                                .getObjEntityView(indexInDataMap - 1));
                    }
                }
            }
            ...
            if (view.getObjEntityViewFieldCount() == 0) {
                ...
                if (index == 0) {
                    selectField(dataViewTabSelected, view
                            .getObjEntityViewField(0));
                } else {
                    selectField(dataViewTabSelected, view
                            .getObjEntityViewField(index - 1));
    ...
    private void selectDataView(DataView dataView) {
        ...
        DataViewTreeModel dataViewTreeModel = (DataViewTreeModel) dataViewTree
                .getModel();
        Object[] path = new Object[] { dataViewTreeModel.getRoot(), dataView };
    ...
    private void selectObjEntity(DVObjEntity entity) {
        ...
        Object[] path = new Object[] { dataMapTreeModel.getRoot(), dataMap,
                entity };

        dataMapTree.setSelectionPath(new TreePath(path));
    }

    private void selectObjEntityView(boolean dataViewTabSelected,
            ObjEntityView view) {
        DataView dataView = view.getDataView();
        JTree dataMapTree = dvModelerFrame.getDataMapTree();
        JTree dataViewTree = dvModelerFrame.getDataViewTree();
    ...
    private void selectField(boolean dataViewTabSelected,
    ...
    private void dataMapTreeNodeSelected() {
        ...
        if (node == null) {
            ...
            showBottomEmptyPanel();
            topTitle = "'" + node + "'  DataView Properties";
            
        } else if (node instanceof DVObjEntity) {
            dvModelerFrame.getTopCardPanel().showPanel((DVObject) node);
            DVObjEntity objEntity = (DVObjEntity) node;
            showRelatioshipsTable(objEntity);
            topTitle = "'" + node + "'  Entity Attributes";
            bottomTitle = "'" + node + "'  Entity Relationships";
            
        } else if (node instanceof ObjEntityView) {
            dvModelerFrame.getTopCardPanel().showPanel((DVObject) node);
            showFieldsTable((ObjEntityView) node);
            topTitle = "'" + node + "\"  Properties";
            bottomTitle = "'" + node + "'  Fields";
            ...
            int index = field.getObjEntityView().getIndexOfObjEntityViewField(
                    field);
            JTable fieldsTable = dvModelerFrame.getFieldsTable();
            fieldsTable.setRowSelectionInterval(index, index);

            topTitle = "'" + node + "'  field properties";
            bottomTitle = "'"
    ...
    private void showRelatioshipsTable(DVObjEntity objEntity) {
        ...
        relationships.addAll(objEntity.getDataMap()
                .getObjRelationshipsBySource(objEntity));
        relationships.addAll(objEntity.getDataMap()
                .getObjRelationshipsByTarget(objEntity));

        JTable relationshipsTable = dvModelerFrame.getRelationshipsTable();
        RelationshipsTableModel model = (RelationshipsTableModel) relationshipsTable
                .getModel();
    ...
    private void configureFileChoosers() {
        ...
        openProjectFileChooser
                .setApproveButtonToolTipText("Open Selected Cayenne Project");
        openProjectFileChooser.setDialogTitle("Open Cayenne Project");
        ...
        public void propertyChange(PropertyChangeEvent e) {
            ...
            if (source instanceof DataView) {
                if ("name".equals(propertyName)) {
                    String topTitle = "Data View \"" + source
                            + "\" :: Properties";
                    dvModelerFrame.getTopTile().setCaption(topTitle);
                ...
                if ("name".equals(propertyName)) {
                    String topTitle = "View \"" + source + "\" :: Properties";
                    String bottomTitle = "View \"" + source + "\" :: Fields";
                    dvModelerFrame.getTopTile().setCaption(topTitle);
                    dvModelerFrame.getBottomTile().setCaption(bottomTitle);
                    dataViewTreeModel
                            .objEntityViewChanged((ObjEntityView) source);
                    dataMapTreeModel
                            .objEntityViewChanged((ObjEntityView) source);
                }
                ...
                if ("name".equals(propertyName)) {
                    ...
                    fieldsTableModel.fireTableCellUpdated(field
                            .getObjEntityView().getIndexOfObjEntityViewField(
                                    field), 0);
                }
    ...
    private void showExceptionDialog(String description, Exception ex) {
        ...
        if (cause != null) {
            message += "caused by " + cause.getClass() + ": "
                    + cause.getMessage();
        }
        message += "</html>";
        JOptionPane.showMessageDialog(dvModelerFrame, message, "Error",
    ...
    public void loadPrefs() {
        ...
        int height = prefs.getInt("height", 350);
        dvModelerFrame.setLocation(x, y);
        dvModelerFrame.setSize(width, height);

        String filename = prefs.get("cayenneProject", null);
        if (filename != null) {
